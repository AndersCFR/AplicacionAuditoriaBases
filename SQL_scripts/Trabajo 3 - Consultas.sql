...
BEGIN
	...
	FROM (
		/* Columnas que no apuntan a un padre y al tener nombres similares pueden implicar que falta una relaci�n */
		SELECT
			table_name, column_name
		FROM (
			/* Columnas que tienen m�s de 1 ocurrencia con su respectiva tabla */
			SELECT
				columnas.column_name,
				columnas.table_name
			FROM (
				/* Columnas que tienen m�s de 1 ocurrencia */
		...
		FULL JOIN (
			/* Restricciones de clave for�nea con sus respectivas columnas */
			SELECT
	...
	LEFT JOIN (
		...
		SELECT
			[Columna PK],
			/* Concatenaci�n de los nombres de las tablas */
			STUFF((
				SELECT ', ' + Tabla
	...
	/* Se buscan tablas con las que podr�a necesitarse una relaci�n */
